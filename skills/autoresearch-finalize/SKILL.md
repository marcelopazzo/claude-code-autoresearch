...
6. Group kept commits into logical changesets:
   ...
   - **Watch for cross-file dependencies.** Each branch is independent, so if group 1 adds an API in `api.rb` and group 2 calls it in `controller.rb`, group 2's branch won't work in isolation. When proposing groups, flag dependencies: "group 2 depends on group 1 — review together." If the dependency is tight, merge the groups.
...
1. **Parallelize test groups** (commits abc1234, def5678)
   Files: Rakefile, test/test_helper.rb
   Metric: 42.3s → 38.1s (-9.9%)

2. **Cache bootsnap for test environment** (commits ghi9012, jkl3456)
  ...
  "groups": [
    {
      ...
      "body": "Why + what changed.\n\nExperiments: #3, #5\nMetric: total_seconds 42.3s → 38.1s (-9.9%)",
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/skills/autoresearch-finalize/finalize.sh /tmp/groups.json
```
